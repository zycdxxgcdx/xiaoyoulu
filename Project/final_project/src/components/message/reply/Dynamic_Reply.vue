  ...
  <div style="height: 100%;">
    ...
    <div v-loading="loading"  infinite-scroll-delay="1000" element-loading-background="#f3f9f6"  class="bottom_content" v-infinite-scroll="load" :infinite-scroll-disabled="disabled" style="overflow-y:auto;padding-left: 1%;padding-top: 1%;box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1)">
      <el-scrollbar style="height: 100%;overflow-x: hidden">
        <div style="margin:1% 0 0 1%;" v-for="item in messageDynamic"  class="info"  @click="toDynamic(item.dynamicId)">
          ...
          <div style="float: left;margin-left: 1%;width: 70%">
            <div style="width: 100%">
              ...
              <span style="color:rgba(0,0,0,.5);font-size: 14px">评论了我的动态</span>
            </div>
  ...
  export default {
    ...
    methods:{
      ...
      load(){
        ...
        _this.loadInfo =true;
        setTimeout(()=>{
          axios.get('http://localhost:8181/message/findDynamicInfo/'+this.$route.params.userId+'/'+this.replyPage+'/'+this.replyPageSize).then(function (resp) {
            console.log(resp);
            if(resp.data.msg === '查询成功'){
              for(let item in resp.data.messageDynamic){
                resp.data.messageDynamic[item].headPosition = 'http://localhost:8181/profilephoto/'+resp.data.messageDynamic[item].headPosition
              }
              _this.messageDynamic = _this.messageDynamic.concat(resp.data.messageDynamic);
              _this.replyPage++;
              if(resp.data.messageDynamic.length < 6){
                _this.disabled = true;
    ...
    created() {
      ...
      axios.get('http://localhost:8181/message/findDynamicInfo/'+this.$route.params.userId+'/1/'+this.replyPageSize).then(function (resp) {
        console.log(resp);
        if(resp.data.msg === '查询成功'){
          for(let item in resp.data.messageDynamic){
            resp.data.messageDynamic[item].headPosition = 'http://localhost:8181/profilephoto/'+resp.data.messageDynamic[item].headPosition
          }
          _this.messageDynamic = resp.data.messageDynamic;
          _this.loading = false;
          if(resp.data.messageDynamic.length < 6){
            _this.disabled = true;
          }
          if(resp.data.messageDynamic.length === 0){
            _this.show = true;
          }
        }
      })
