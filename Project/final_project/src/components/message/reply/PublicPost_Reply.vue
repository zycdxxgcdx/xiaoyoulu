<template>
  <div style="height: 100%;">
    ...
    <div v-loading="loading"  infinite-scroll-delay="1000" element-loading-background="#f3f9f6"  class="bottom_content" v-infinite-scroll="load" :infinite-scroll-disabled="disabled" style="overflow-y:auto;padding-left: 1%;padding-top: 1%;box-shadow: 0 2px 12px 0 rgba(0, 0, 0, 0.1)">
      ...
      <div style="margin:1% 0 0 1%;" v-for="item in messagePost"  class="info"  @click="toPublicPost(item.publicpostid)">
        ...
        <div style="float: left;margin-left: 1%;width: 70%">
          <div style="width: 100%">
            ...
            <span style="color:rgba(0,0,0,.5);font-size: 14px">评论了我的帖子</span>
          </div>
        ...
        <div style="overflow:hidden;text-overflow: ellipsis;width: 10%;float: right;margin-right: 2%;color:rgba(0,0,0,.5);font-size: 13px">{{item.toContent}}</div>
  ...
  export default {
    ...
    methods:{
      ...
      load(){
        ...
        _this.loadInfo =true;
        setTimeout(()=>{
          axios.get('http://localhost:8181/message/findPostInfo/'+this.$route.params.userId+'/'+this.replyPage+'/'+this.replyPageSize).then(function (resp) {
            console.log(resp);
            if(resp.data.msg === '查询成功'){
              for(let item in resp.data.messagePost){
                resp.data.messagePost[item].headPosition = 'http://localhost:8181/profilephoto/'+resp.data.messagePost[item].headPosition
              }
              // _this.loadInfo =true;
              _this.messagePost = _this.messagePost.concat(resp.data.messagePost);
              _this.replyPage++;
              if(resp.data.messagePost.length < 6){
                _this.disabled = true;
    ...
    created() {
      ...
      axios.get('http://localhost:8181/message/findPostInfo/'+this.$route.params.userId+'/1/'+this.replyPageSize).then(function (resp) {
        console.log(resp);
        if(resp.data.msg === '查询成功'){
          for(let item in resp.data.messagePost){
            resp.data.messagePost[item].headPosition = 'http://localhost:8181/profilephoto/'+resp.data.messagePost[item].headPosition
          }
          _this.messagePost = resp.data.messagePost;
          _this.loading = false;
          if(resp.data.messagePost.length < 6){
            _this.disabled = true;
          }
          if(resp.data.messagePost.length === 0){
            _this.show = true;
          }
        }
      })
